...
<div class="row">
    <div class="col-md-12">
        <div class="card card-primary">
            <div class="card-body">
                <form action=" {{route('types.update',['type'=>$type->id])}} " method="post" id="submit">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="form-group">
                                <label for="type">@lang('site.task_type')</label>
                                <input type="text" class="form-control 
                                @error('type') is-invalid @enderror " 
                                id="type" name="type" value="{{ old('type',$type->type) }}" 
                                @error('type')aria-invalid="true"@enderror
                                placeholder="@lang('site.task_type')" required>
                ...
                {{--  cancel button --}}
                <form action="{{route('types.index')}}" method="get" id="cancel" class="d-none">
                @csrf
                </form>
            </div>
        <!-- /.card-body -->
